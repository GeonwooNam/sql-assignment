...
#
# 그래서 정합성을 15점으로 내리고, 빠진 15점으로 "질문의 질"을 새로 세웠다.
...
>    거기에 배점을 크게 주지 않는다(15점). 점수는 **어떤 질문을 세웠는지(20점)** 와
>    **그 질문에 답하려고 무엇을 고민했는지(25점)** 에서 갈린다.
>
> ### AI를 써도 됩니다. 대신 알아두세요
>
...
| 단계 | 점수 | 기준 |
|---|---|---|
| 1 | 0–5 | 질문이 없다. 또는 이 데이터로 답할 수 없다. 또는 답이 이미 자명하다 ("주문이 가장 많은 주는?") |
| 2 | 6–11 | 답할 수 있고 자명하지도 않지만 **막연하다.** 대상이나 비교 기준이 특정되지 않음 ("배송과 리뷰의 관계를 보고 싶다") |
| 3 | 12–16 | **대상 · 비교축 · 판단 기준이 특정됨.** 답이 나오면 무엇을 알게 되는지가 분명하다 |
| 4 | 17–20 | 위에 더해 **답이 어느 쪽으로 나와도 의미가 있고, 데이터를 한 번 본 뒤에야 나올 수 있는 질문.** 특정 카테고리·구간·세그먼트를 지목해 통념의 반례를 찌른다. 데이터를 안 보고도 쓸 수 있는 질문은 3단계까지다 |
...
| 4 | 20–25 | 위에 더해 **왜 그렇게 선택했는지** 와 **그 선택 때문에 무엇이 보이지 않게 되는지**(잃은 것)를 함께 밝힘. 예: "취소 건을 뺐으므로 취소가 잦은 판매자의 문제는 이 분석에 안 나타난다". 이유만 있고 대가가 없으면 3단계다 |
...
**어긋나면 잃는 점수**다. 그래서 배점을 낮게 뒀다. 대신 아래 연쇄 차단이 걸린다.

| 단계 | 점수 | 기준 |
|---|---|---|
| 1 | 0–4 | 사슬이 끊김. 쿼리가 질문과 다른 것을 계산하거나, 주장이 결과에 없는 내용임 |
| 2 | 5–8 | 질문과 쿼리가 대체로 맞지만 **범위가 어긋남** (아래 예시 참고) |
...
답하지 못하는 쿼리에서 나온 깊이와 해석은 근거가 없기 때문이다. 배점이 15점으로 낮아도
어긋남의 대가가 작지 않은 이유다.
...
따라서 근거 사슬 정합성은 15점뿐이다. 여기서 만점을 받았다고 좋은 제출물이 아니다.

점수는 두 곳에서 갈린다.
1. **질문의 질(20점)** — 시시한 질문을 정확하게 답한 제출물에 높은 점수를 주지 마라.
   집계 한 번으로 끝나는 질문, 답을 알아도 할 일이 없는 질문은 1~2단계다.
   질문이 구체적인지(대상·비교축·판단 기준이 특정되었는지)를 보고,
   답이 어느 쪽으로 나와도 의미가 있는 질문인지를 봐라.
2. **쿼리 설계 판단(25점)** — 질문에 답하려고 무엇을 고민했는지.
...
- 분석의 깊이 → 2단계를 넘지 못한다 (최대 11점)
- 비즈니스 해석 → 2단계를 넘지 못한다 (최대 7점)
질문의 질과 쿼리 설계 판단은 이 차단을 받지 않는다. 좋은 질문을 세웠다는 사실 자체는
쿼리가 틀렸어도 남아 있기 때문이다.
</연쇄 차단 규칙>
